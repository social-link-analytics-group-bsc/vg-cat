...
class CatalanGrammarChecker:
    def __init__(self):
        token = os.getenv("HUGGINGFACE_TOKEN")  # NO hardcodear tokens
        self.model_name = "BSC-LT/roberta-base-ca"
        # Pasa el token sólo si existe
        kwargs = {"token": token} if token else {}
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_name, **kwargs)
        self.model = AutoModelForMaskedLM.from_pretrained(self.model_name, **kwargs)
